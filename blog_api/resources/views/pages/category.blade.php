			...
			<div class="about-main">
				<div class="col-md-8 about-left">
					<div class="about-one">
						...
						<p style="font-size: 13px;">{{$category_post_new->category->title_category}}</p>
						<h3>{{$category_post_new->title_post}}</h3>
						@endif
					</div>

					<div class="about-two">
						@if(count($category_post) != 0)
						<a href="{{route('bai-viet.show',['id'=>$category_post_new->id_post,'slug'=>Str::slug($category_post_new->title_post)])}}"><img src="{{asset('public/uploads/'.$category_post_new->image)}}" alt="{{Str::slug($category_post_new->title_post)}}" width="730px" height="417px" /></a>
						<p>Đăng bởi <a href="#">Pi Pj</a> Ngày {{$category_post_new->date}} <a href="#">Lượt xem ({{$category_post_new->post_view}})</a></p>
						<p>{!! substr($category_post_new->short_desc,0,250) !!}...</p>
						<div class="about-btn">
							<a href="{{route('bai-viet.show',['id'=>$category_post_new->id_post,'slug'=>Str::slug($category_post_new->title_post)])}}">Chi Tiết</a>
						</div>
						@endif
						<ul>
							<li><p>Chia Sẻ : </p></li>
					...
					<div class="about-tre">
						@foreach($category_post as $cate_pst)
